...
```bash
cargo build --profile release-fast -p freeco-cli
```
...
```bash
cargo test -p freeco-kernel
cargo test -p freeco-kernel-runtime
cargo test -p freeco-memory
```
...
- **Naming**:
  - Types: `PascalCase` (e.g., `FreecoKernel`, `AgentManifest`)
  - Functions/methods: `snake_case`
  - Constants: `SCREAMING_SNAKE_CASE`
  - Crate names: `freeco-{name}` (kebab-case)
...
Freeco is organized as a Cargo workspace with 14 crates:

| Crate | Role |
|-------|------|
| `freeco-types` | Shared type definitions, taint tracking, manifest signing (Ed25519), model catalog, MCP/A2A config types |
| `freeco-memory` | SQLite-backed memory substrate with vector embeddings, usage tracking, canonical sessions, JSONL mirroring |
...
| `freeco-hands` | Hands system (curated autonomous capability packages), 7 bundled hands |
| `freeco-extensions` | Integration registry (25 bundled MCP templates), AES-256-GCM credential vault, OAuth2 PKCE |
| `freeco-kernel` | Assembles all subsystems: workflow engine, RBAC auth, heartbeat monitor, cron scheduler, config hot-reload |
| `freeco-api` | REST/WS/SSE API (Axum 0.8), 76 endpoints, 14-page SPA dashboard, OpenAI-compatible `/v1/chat/completions` |
| `freeco-channels` | 40 channel adapters (Telegram, Discord, Slack, WhatsApp, and 36 more), formatter, rate limiter |
| `freeco-wire` | OFP (Freeco Protocol): TCP P2P networking with HMAC-SHA256 mutual authentication |
| `freeco-cli` | Clap CLI with daemon auto-detect (HTTP mode vs. in-process fallback), MCP server |
| `freeco-migrate` | Migration engine for importing from OpenClaw (and future frameworks) |
| `freeco-skills` | Skill system: 60 bundled skills, FangHub marketplace, OpenClaw compatibility, prompt injection scanning |
| `freeco-desktop` | Tauri 2.0 native desktop app (WebView + system tray + single-instance + notifications) |
| `xtask` | Build automation tasks |

### Key Architectural Patterns

- **`KernelHandle` trait**: Defined in `freeco-kernel-runtime`, implemented on `FreecoKernel` in `freeco-kernel`. This avoids circular crate dependencies while enabling inter-agent tools.
- **Shared memory**: A fixed UUID (`AgentId(Uuid::from_bytes([0..0, 0x01]))`) provides a cross-agent KV namespace.
- **Daemon detection**: The CLI checks `~/.freeco-ai/daemon.json` and pings the health endpoint. If a daemon is running, commands use HTTP; otherwise, they boot an in-process kernel.
...
Channel adapters live in `crates/freeco-channels/src/`. Each adapter implements the `ChannelAdapter` trait.

### Steps

1. Create a new file: `crates/freeco-channels/src/myplatform.rs`
...
3. Register the module in `crates/freeco-channels/src/lib.rs`:

```rust
pub mod myplatform;
```

4. Wire it up in the channel bridge (`crates/freeco-api/src/channel_bridge.rs`) so the daemon starts it alongside other adapters.

5. Add configuration support in `freeco-types` config structs (add a `[channels.myplatform]` section).

6. Add CLI setup wizard instructions in `crates/freeco-cli/src/main.rs` under `cmd_channel_setup`.
...
- Open a [GitHub Discussion](https://github.com/FreecoDAO/FreEco-ai/discussions) for questions.
- Open a [GitHub Issue](https://github.com/FreecoDAO/FreEco-ai/issues) for bugs or feature requests.
