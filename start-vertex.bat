...
set RUST_LOG=freeco_kernel_runtime::drivers::vertex=debug,freeco=info
set RUST_BACKTRACE=full
cd /d C:\Users\at384\Downloads\osc\dllm\freeco
echo Getting access token...
for /f "tokens=*" %%a in ('gcloud auth print-access-token') do set VERTEX_AI_ACCESS_TOKEN=%%a
echo Token set, starting Freeco...
target\debug\freeco.exe start
pause
