//! Minimal `.env` file loader/saver for `~/.freeco-ai/.env`.
//!
...
/// Remove a key from `~/.freeco-ai/.env`.
///
...
/// List key names (without values) from `~/.freeco-ai/.env`.
#[allow(dead_code)]
...
fn write_env_file(path: &PathBuf, entries: &BTreeMap<String, String>) -> Result<(), String> {
    let mut content =
        String::from("# Freeco environment — managed by `freeco-ai config set-key`\n");
