...
pub struct InProcessAgent {
    pub id: freeco_types::agent::AgentId,
    pub name: String,
    pub state: String,
    pub provider: String,
    pub model: String,
...
impl AgentSelectState {
    ...
    pub fn load_inprocess_agents(&mut self, kernel: &freeco_kernel::FreecoKernel) {
...
fn truncate(s: &str, max: usize) -> String {
    if s.len() <= max {
        ...
        format!(
            "{}\u{2026}",
            freeco_types::truncate_str(s, max.saturating_sub(1))
        )
    }
}
