//! Cron job scheduler engine for the Freeco kernel.
//!
...
use freeco_types::scheduler::{CronJob, CronJobId, CronSchedule};
...
///
/// The `CronJob` struct in `freeco-types` is intentionally lean (no
...
impl CronScheduler {
    ...
    ///
    /// `home_dir` is the Freeco data directory; jobs are persisted to
    ...
    pub fn load(&self) -> FreecoResult<usize> {
        if !self.persist_path.exists() {
            return Ok(0);
        }
        let data = std::fs::read_to_string(&self.persist_path)
            .map_err(|e| FreecoError::Internal(format!("Failed to read cron jobs: {e}")))?;
        let metas: Vec<JobMeta> = serde_json::from_str(&data)
            .map_err(|e| FreecoError::Internal(format!("Failed to parse cron jobs: {e}")))?;
        let count = metas.len();
        for meta in metas {
    ...
    pub fn add_job(&self, mut job: CronJob, one_shot: bool) -> FreecoResult<CronJobId> {
        // Global limit
        let max_jobs = self.max_total_jobs.load(Ordering::Relaxed);
        if self.jobs.len() >= max_jobs {
            return Err(FreecoError::Internal(format!(
        ...
        job.validate(agent_count)
            .map_err(FreecoError::InvalidInput)?;
    ...
    pub fn remove_job(&self, id: CronJobId) -> FreecoResult<CronJob> {
        self.jobs
            .remove(&id)
            .map(|(_, meta)| meta.job)
            .ok_or_else(|| FreecoError::Internal(format!("Cron job {id} not found")))
    }

    /// Enable or disable a job. Re-enabling resets errors and recomputes
    /// `next_run`.
    pub fn set_enabled(&self, id: CronJobId, enabled: bool) -> FreecoResult<()> {
        match self.jobs.get_mut(&id) {
            ...
            None => Err(FreecoError::Internal(format!("Cron job {id} not found"))),
        }
    }
    ...
    pub fn set_delivery_targets(
        &self,
        id: CronJobId,
        targets: Vec<freeco_types::scheduler::CronDeliveryTarget>,
    ) -> FreecoResult<()> {
        match self.jobs.get_mut(&id) {
            Some(mut meta) => {
    ...
    pub fn record_failure(&self, id: CronJobId, error_msg: &str) {
        if let Some(mut meta) = self.jobs.get_mut(&id) {
            ...
            meta.last_status = Some(format!(
                "error: {}",
                freeco_types::truncate_str(error_msg, 256)
            ));
...
mod tests {
    ...
    use freeco_types::scheduler::{CronAction, CronDelivery};
    ...
    fn test_add_job_per_agent_limit() {
        // MAX_JOBS_PER_AGENT = 50 in freeco-types
