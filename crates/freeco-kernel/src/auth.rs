...
//! to Freeco users with roles, then enforces permission checks on actions.

use dashmap::DashMap;
use freeco_types::agent::UserId;
...
pub struct AuthManager {
    /// Known users by their Freeco user ID.
...
impl AuthManager {
    ...
    ///
    /// Returns the Freeco UserId if a matching channel binding exists,
    ...
    pub fn authorize(&self, user_id: UserId, action: &Action) -> FreecoResult<()> {
        let identity = self
            .users
            .get(&user_id)
            .ok_or_else(|| FreecoError::AuthDenied("Unknown user".to_string()))?;

        let required = action.required_role();
        if identity.role >= required {
            Ok(())
        } else {
            Err(FreecoError::AuthDenied(format!(
