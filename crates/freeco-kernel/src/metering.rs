...
use freeco_memory::usage::{ModelUsage, UsageRecord, UsageStore, UsageSummary};
use freeco_types::agent::{AgentId, ResourceQuota};
...
impl MeteringEngine {
    ...
    pub fn check_quota(&self, agent_id: AgentId, quota: &ResourceQuota) -> FreecoResult<()> {
        // Hourly check
        if quota.max_cost_per_hour_usd > 0.0 {
            ...
            if hourly_cost >= quota.max_cost_per_hour_usd {
                return Err(FreecoError::QuotaExceeded(format!(
    ...
    pub fn budget_status(&self, budget: &freeco_types::config::BudgetConfig) -> BudgetStatus {
    ...
    pub fn get_summary(&self, agent_id: Option<AgentId>) -> FreecoResult<UsageSummary> {
        self.store.query_summary(agent_id)
    }

    /// Get usage grouped by model.
    pub fn get_by_model(&self) -> FreecoResult<Vec<ModelUsage>> {
    ...
    pub fn cleanup(&self, days: u32) -> FreecoResult<usize> {
