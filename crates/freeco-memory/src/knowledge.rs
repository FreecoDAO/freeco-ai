...
impl KnowledgeStore {
    ...
    pub fn add_entity(&self, entity: Entity) -> FreecoResult<String> {
        let conn = self
            .conn
            .lock()
    ...
    pub fn add_relation(&self, relation: Relation) -> FreecoResult<String> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| FreecoError::Internal(e.to_string()))?;
    ...
    pub fn query_graph(&self, pattern: GraphPattern) -> FreecoResult<Vec<GraphMatch>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| FreecoError::Internal(e.to_string()))?;
        ...
        for row_result in rows {
            let r = row_result.map_err(|e| FreecoError::Memory(e.to_string()))?;
