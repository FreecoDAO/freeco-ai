// FreEco.ai Comms Page — Agent topology & inter-agent communication feed
'use strict';
...
function commsPage() {
  return {
    ...
    async loadData() {
      ...
      try {
        var results = await Promise.all([
          FreecoAPI.get('/api/comms/topology'),
          FreecoAPI.get('/api/comms/events?limit=200')
        ]);
    ...
    startSSE() {
      ...
      if (FreecoAPI.apiKey) url += '?token=' + encodeURIComponent(FreecoAPI.apiKey);
    ...
    async refreshTopology() {
      try {
        this.topology = await FreecoAPI.get('/api/comms/topology');
      } catch(e) { /* silent */ }
    },
    ...
    async submitSend() {
      ...
      try {
        await FreecoAPI.post('/api/comms/send', {
          from_agent_id: this.sendFrom,
          to_agent_id: this.sendTo,
          message: this.sendMsg
        });
        FreecoToast.success('Message sent');
        this.showSendModal = false;
      } catch(e) {
        FreecoToast.error(e.message || 'Send failed');
      }
    ...
    async submitTask() {
      ...
      try {
        ...
        await FreecoAPI.post('/api/comms/task', body);
        FreecoToast.success('Task posted');
        this.showTaskModal = false;
      } catch(e) {
        FreecoToast.error(e.message || 'Task failed');
      }
