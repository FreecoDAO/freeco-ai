...
//!
//! Allows any OpenAI-compatible client library to talk to Freeco agents.
//! The `model` field resolves to an agent (by name, UUID, or `freeco:<name>`),
...
use freeco_types::message::{ContentBlock, Message, MessageContent, Role, StopReason};
...
fn resolve_agent(state: &AppState, model: &str) -> Option<(AgentId, String)> {
    // 1. "freeco:<name>" → find agent by name
    if let Some(name) = model.strip_prefix("freeco:") {
...
pub async fn list_models(State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let models: Vec<ModelObject> = agents
        .iter()
        .map(|e| ModelObject {
            id: format!("freeco:{}", e.name),
            object: "model",
            created,
            owned_by: "freeco".to_string(),
        })
        .collect();
