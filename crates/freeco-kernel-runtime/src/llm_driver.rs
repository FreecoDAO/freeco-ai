...
use freeco_types::message::{ContentBlock, Message, StopReason, TokenUsage};
...
pub struct CompletionRequest {
    ...
    pub thinking: Option<freeco_types::config::ThinkingConfig>,
}
...
pub struct DriverConfig {
    ...
    /// `claude` CLI.  Defaults to `true` because Freeco runs as a daemon
    /// with no interactive terminal, so permission prompts would block
    /// indefinitely.  Freeco's own capability / RBAC layer already
