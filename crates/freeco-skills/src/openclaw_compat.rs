...
//!
//! This module detects both formats and converts them to Freeco `SkillManifest`.
...
/// Result of converting a SKILL.md into Freeco format.
#[derive(Debug, Clone)]
pub struct ConvertedSkillMd {
    ...
    /// Tool name translations applied (openclaw_name → freeco_name).
...
/// Full conversion of a SKILL.md directory to Freeco format.
///
...
pub fn convert_skillmd(dir: &Path) -> Result<ConvertedSkillMd, SkillError> {
    ...
    if let Some(ref meta) = frontmatter.metadata.openclaw {
        ...
        // Convert commands to Freeco tool definitions
        for cmd in &meta.commands {
            ...
            let freeco_name = if let Some(mapped) = tool_compat::map_tool_name(&cmd.name) {
                tool_translations.push((cmd.name.clone(), mapped.to_string()));
                mapped.to_string()
            } else if tool_compat::is_known_freeco_tool(&cmd.name) {
                cmd.name.clone()
            } else {
...
/// Convert an in-memory SKILL.md string into Freeco format.
///
...
/// Convert an OpenClaw Node.js skill directory into an Freeco SkillManifest.
///
...
/// Write an Freeco skill.toml manifest for an OpenClaw skill.
pub fn write_freeco_manifest(dir: &Path, manifest: &SkillManifest) -> Result<(), SkillError> {
