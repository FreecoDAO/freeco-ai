...
FreEco.ai implements both the **Model Context Protocol (MCP)** and **Agent-to-Agent (A2A)** protocol, enabling deep interoperability with external tools, IDEs, and other agent frameworks.
...
- [Part 1: MCP (Model Context Protocol)](#part-1-mcp-model-context-protocol)
  ...
  - [MCP Server -- Exposing FreEco.ai via MCP](#mcp-server)
...
The Model Context Protocol (MCP) is a JSON-RPC 2.0 based protocol that standardizes how LLM applications discover and invoke tools. FreEco.ai supports MCP in both directions:

- **As a client**: FreEco.ai connects to external MCP servers (GitHub, filesystem, databases, Puppeteer, etc.) and makes their tools available to all agents.
- **As a server**: FreEco.ai exposes its own agents as MCP tools, so IDEs like Cursor, VS Code, and Claude Desktop can call FreEco.ai agents directly.
...
FreEco.ai supports two MCP transports, defined by `McpTransport`:
...
FreEco.ai can also act as an MCP server, exposing its agents as callable tools to external MCP clients.
...
This command:
1. Checks if an FreEco.ai daemon is running (via `find_daemon()`)
...
- `McpBackend::Daemon` -- forwards requests to a running FreEco.ai daemon via HTTP
- `McpBackend::InProcess` -- boots a full kernel when no daemon is available

#### HTTP MCP Endpoint

FreEco.ai also exposes an MCP endpoint over HTTP at `POST /mcp`. Unlike the stdio server (which only exposes agents), the HTTP endpoint exposes the full tool set (built-in + skills + MCP tools) and executes tools via the kernel's `execute_tool()` pipeline. This means the HTTP MCP endpoint supports:
...
FreEco.ai implements A2A in both directions:
...
pub struct AgentCapabilities {
    pub streaming: bool,                 // true -- FreEco.ai supports streaming
...
**AgentSkill** (not the same as FreEco.ai skills -- these are A2A capability descriptors):

```rust
pub struct AgentSkill {
    pub id: String,           // matches the FreEco.ai tool name
...
Agent Cards are built from FreEco.ai agent manifests via `build_agent_card()`. Each tool in the agent's capability list becomes an A2A skill descriptor. Example card:
...
FreEco.ai serves A2A requests through the REST API. The server-side implementation involves:
...
**External Agent Discovery**: The `A2aClient` uses a 30-second timeout and sends a `User-Agent: FreEco.ai/0.1 A2A` header. Failed discoveries are logged but do not block kernel boot.
