...
- **Raw GitHub redirect**: Use `freeco.ai` as a CNAME to `raw.githubusercontent.com/FreecoDAO/FreEco-ai/main/scripts/install.sh` (less reliable).
...
```bash
curl -sSf https://raw.githubusercontent.com/FreecoDAO/FreEco-ai/main/scripts/install.sh | sh
```
...
Visit: `https://github.com/FreecoDAO/FreEco-ai/releases/latest/download/latest.json`
...
docker pull ghcr.io/freecoda/freeco-ai:0.1.0

# Verify both architectures
docker run --rm ghcr.io/freecoda/freeco-ai:latest --version
```
