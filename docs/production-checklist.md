...
- **Raw GitHub redirect**: Use `freeco.ai` as a CNAME to `raw.githubusercontent.com/FreecoDAO/openfang/main/scripts/install.sh` (less reliable).
...
```bash
curl -sSf https://raw.githubusercontent.com/FreecoDAO/openfang/main/scripts/install.sh | sh
```
...
Visit: `https://github.com/FreecoDAO/openfang/releases/latest/download/latest.json`
...
docker pull ghcr.io/FreecoDAO/openfang:0.1.0

# Verify both architectures
docker run --rm ghcr.io/FreecoDAO/openfang:latest --version
```
