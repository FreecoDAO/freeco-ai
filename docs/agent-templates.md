...
FreEco.ai ships with **38 pre-built agent templates** organized into 4 performance tiers. Each template is a ready-to-spawn `agent.toml` manifest located in the `agents/` directory. Templates cover software engineering, business operations, personal productivity, and everyday tasks.
...
```bash
freeco spawn orchestrator
freeco spawn coder
freeco spawn --template agents/writer/agent.toml
```
...
> General-purpose assistant. The default FreEco.ai agent for everyday tasks, questions, and conversations.
...
```bash
freeco spawn meeting-assistant
...
freeco spawn coder --name "backend-coder"

# Spawn from a TOML file path
freeco spawn --template agents/custom/my-agent.toml

# List running agents
freeco agents

# Send a message
freeco message <agent-id> "Write a function to parse TOML files"

# Kill an agent
freeco kill <agent-id>
```
