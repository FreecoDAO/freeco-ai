"""
Freeco Python SDK — helper library for writing Python agents.
...
Or for simple scripts without the decorator pattern:
    ...
    from freeco_sdk import read_input, respond
...
def read_input() -> Dict[str, Any]:
    """Read the input JSON from stdin (sent by the Freeco kernel)."""
    line = sys.stdin.readline().strip()
    if not line:
        # Fallback: check environment variables
        agent_id = os.environ.get("FREECO_AI_AGENT_ID", "")
        message = os.environ.get("FREECO_AI_MESSAGE", "")
        return {
            "type": "message",
            "agent_id": agent_id,
            "message": message,
...
def respond(text: str, metadata: Optional[Dict[str, Any]] = None) -> None:
    """Send a response back to the Freeco kernel via stdout."""
...
def log(message: str, level: str = "info") -> None:
    """Log a message to stderr (visible in Freeco daemon logs)."""
...
if __name__ == "__main__":
    print("Freeco Python SDK")
    print("====================")
    print()
    print("Import this module in your agent scripts:")
    print()
    print("  from freeco_sdk import Agent")
    print()
    print("  agent = Agent()")
    print()
